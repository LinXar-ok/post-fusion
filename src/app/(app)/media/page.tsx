...
export default function MediaLibraryPage() {
  ...
  const [refs, setRefs] = useState<Map<string, HTMLInputElement | null>>(
    new Map(),
  );
  ...
  const loadFiles = async (uid: string) => {
    ...
    setFiles(
      (data?.map((f) => ({ ...f, id: f.id ?? "" })) as MediaFile[]) ?? [],
    );
    setLoading(false);
  };
              ...
              {filteredFiles.map((file, i) => (
                ...
                >
                  <img
                    src={
                      userId
                        ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/media/${userId}/${file.name}`
                        : ""
                    }
                    alt={file.name}
