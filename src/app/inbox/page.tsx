  ...
  return (
    <div className="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto w-full h-[calc(100vh-4rem)] flex flex-col relative z-10">
      ...
      <Card className="flex-1 bg-white border-slate-200 shadow-sm backdrop-blur-xl overflow-hidden flex min-h-0">
          ...
          <div className="p-4 border-b border-slate-100 bg-white">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 bg-slate-50 border-slate-200 shadow-xs h-9 text-sm rounded-lg focus-visible:ring-violet-500"
              />
            </div>
            <div className="flex gap-2 mt-3 overflow-x-auto pb-1 hide-scrollbar">
              <Button variant="secondary" size="sm" className="h-7 text-xs rounded-full bg-violet-100 hover:bg-violet-200 text-violet-700 font-bold border border-violet-200 shadow-xs shrink-0">
                All (12)
              </Button>
              <Button variant="outline" size="sm" className="h-7 text-xs rounded-full bg-white hover:bg-slate-50 text-slate-600 font-semibold border-slate-200 shadow-xs shrink-0">
                Unread (2)
              </Button>
          ...
          <div className="overflow-y-auto flex-1">
            {messages.map((msg) => (
              <div key={msg.id} className={`p-4 border-b border-slate-100 cursor-pointer transition-colors ${msg.unread ? 'bg-white hover:bg-slate-50' : 'hover:bg-slate-100/50 opacity-80'}`}>
                ...
                <p className={`text-sm mt-2 line-clamp-2 leading-relaxed ${msg.unread ? 'text-slate-800 font-medium' : 'text-slate-500'}`}>
                  {msg.excerpt}
                </p>
                {msg.unread && (
                  <div className="w-2 h-2 rounded-full bg-violet-600 mt-3 shadow-xs" />
                )}
              </div>
            ))}
          </div>
        ...
        {/* Right Content: Selected Thread */}
        <div className="hidden md:flex flex-1 flex-col bg-white h-full relative">
          <div className="h-16 border-b border-slate-100 bg-white/80 backdrop-blur-xl flex items-center px-6 shrink-0 relative z-10">
            ...
            <div className="flex flex-col space-y-6">
              ...
              <div className="bg-white p-4 rounded-2xl rounded-tl-sm border border-slate-200 shadow-sm max-w-lg self-start">
                <p className="text-slate-700 text-[15px] leading-relaxed">
                  Hey there! I saw the announcement about the new scheduling feature. Absolutely love the layout! When exactly is the new feature dropping for beta users?
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-slate-100 bg-white shrink-0">
            <div className="relative flex items-center">
              <Input
                ...
                className="flex-1 pr-12 bg-slate-50 border-slate-200 text-slate-900 h-12 rounded-xl shadow-inner focus-visible:ring-violet-500"
              />
              <Button size="icon" className="absolute right-1.5 h-9 w-9 rounded-lg bg-violet-600 hover:bg-violet-700 shadow-sm text-white transition-transform hover:scale-105">
